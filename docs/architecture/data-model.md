...
`SupplierAttachment` es una colección 1:N: una captura DRAFT puede conservar múltiples `BUSINESS_CARD`, `PRODUCT_IMAGE` y `AUDIO`. La captura es la raíz de ownership del adjunto; a través de ella se resuelven viaje y autor, evitando duplicar identidad en la tabla de metadata. Las fotos de producto son evidencia visual y no ingresan al OCR de tarjetas.
