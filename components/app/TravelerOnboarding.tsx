...
import { ArrowRight, Check, ImageIcon, LoaderCircle, Mic, Phone, Sparkles, Type } from "lucide-react";
...
export function TravelerOnboarding({ tripId }: { tripId: string }) {
  ...
  async function complete() {
    ...
    try { if (whatsappPhone.trim()) await appApi(`/api/bot/trips/${encodeURIComponent(tripId)}/whatsapp`, { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify({ whatsappPhone }) }); await appApi(`/api/bot/trips/${encodeURIComponent(tripId)}/onboarding`, { method: "POST" }); router.replace(`/app/viajes/${tripId}`); router.refresh(); }
  ...
  const screens = [
    <section key="welcome" className="space-y-5"><div className="grid h-16 w-16 place-items-center rounded-2xl bg-nihao text-white"><Sparkles className="h-8 w-8" /></div><div><p className="text-eyebrow-mark">Tu viaje empieza acá</p><h1 className="mt-3 text-4xl leading-tight text-ink">Bienvenido a Nihao</h1><p className="mt-3 text-lg text-ink-mute">{state.tripName}</p></div><p className="text-base leading-7 text-ink-soft">Durante la feria vas a poder guardar información de tus proveedores en pocos segundos.</p><label className="block"><span className="flex items-center gap-2 text-sm font-semibold text-ink"><Phone className="h-4 w-4 text-nihao" />Tu WhatsApp (opcional)</span><input value={whatsappPhone} onChange={(event) => setWhatsappPhone(event.target.value)} inputMode="tel" autoComplete="tel" placeholder="5493412345678" className="mt-2 min-h-12 w-full rounded-xl border border-line px-3 text-ink" /><span className="mt-1 block text-xs leading-5 text-ink-mute"><span className="block font-medium text-ink-soft">Ejemplo Argentina: 5493412345678</span>54 país + 9 + código de área + número. Sin +, espacios ni guiones.</span></label></section>,
