...
function WhatsAppCard({ phone, saved, onChange, onSave }: { phone: string; saved: boolean; onChange: (value: string) => void; onSave: () => Promise<void> }) {
  ...
  return <section className="mt-6 rounded-2xl border border-line bg-white p-4 shadow-soft"><div className="flex gap-3"><Phone className="mt-0.5 h-5 w-5 text-nihao" /><div><h2 className="font-semibold">WhatsApp</h2><p className="mt-1 text-sm text-ink-mute">Vinculalo para enviar capturas por texto. Incluí el código de país.</p></div></div><div className="mt-3 flex gap-2"><input value={phone} onChange={(event) => onChange(event.target.value)} inputMode="tel" placeholder="5493412345678" className="min-h-11 min-w-0 flex-1 rounded-xl border border-line px-3 text-sm" /><button type="button" disabled={busy} onClick={() => { setBusy(true); setError(null); void onSave().catch((caught) => setError(caught instanceof Error ? caught.message : "No pudimos guardar el número")).finally(() => setBusy(false)); }} className="app-secondary-button px-3 text-sm">Guardar</button></div><p className="mt-2 text-xs leading-5 text-ink-mute"><span className="block font-medium text-ink-soft">Ejemplo Argentina: 5493412345678</span>54 país + 9 + código de área + número. Sin +, espacios ni guiones.</p>{error ? <p role="alert" className="mt-2 text-xs text-nihao">{error}</p> : saved ? <p className="mt-2 text-xs text-ink-mute">WhatsApp vinculado.</p> : null}</section>;
}
